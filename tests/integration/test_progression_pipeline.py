...
class FakeStatsSystem:
    """01-stats-combat.md's StatsSystem isn't merged yet (CONTRACTS.md §8
    stubbing) -- this fake serves the same public shape."""

    def __init__(self, stats: dict[str, float] | None = None) -> None:
        self._stats = stats or {}
        self.modifier_calls: list[tuple] = []
    ...
    def add_modifier(self, entity_id: int, tag: str, stat: str, op: str, value: float) -> None:
        self.modifier_calls.append((entity_id, tag, stat, op, value))
...
def test_progression_pipeline_multi_level_up_with_real_registry_and_fixture_spells():
    ...
    # Automatic stat gains actually reached StatsSystem.add_modifier.
    auto_tags = [call[1] for call in stats.modifier_calls if call[1].startswith("levelup_auto_")]
...
def test_vision_pipeline_lit_room_then_corridor_via_real_player_moved_event():
    ...
    stats = FakeStatsSystem({"vision_range": 2, "trap_detect_radius": 0})
